...
class Match:
    def __init__(self, players, deck, table):
        ...
        while "Exploding Kitten" in self.deck.deck:
            bombs.append(
                self.deck.deck.pop(self.deck.deck.index("Exploding Kitten"))
            )
    ...
    def turn(self):
        ...
        if self.table.attack_cards:
            ...
            if attack_card == "Attack":
                print(
                    f"Do you want to use one of your cards? [y/n]\n\n"
                    f"Your cards are \n{player.cards}"
                )
                answer = input()
                ...
                if answer == "y":
                    print(
                        f"Which card do you want to use? [Reply with a number]"
                        f"\n\nYour cards are \n"
                    ...
                    while answer != "n":
                        print(
                            f"Which card do you want to use? (Please in this case reply y just if you want to use Nope or Attack) [Reply with a number]\n\nYour cards are \n{[(i+1, player.cards[i]) for i in range(len(player.cards))]}"
                        )
                        y = int(input())
                        used_card = player.cards.pop(y - 1)
                        self.table.attack_cards.append(used_card)
                    ...
                    if used_card == "Attack":
                        ...
                        number_of_nope = sum(
                            [1 for x in used_cards if x == "Nope"]
                        )
                        print(f"There have been used {number_of_nope} Nopes")
                        if (
                            number_of_nope < len(self.table.attack_cards)
                            and "Attack" in self.table.attack_cards
                        ):
                            self.table.attack_card = self.table.attack_cards[
                                :-number_of_nope
                            ]
                            print(
                                f"Attack cards are still {self.table.attack_cards}"
                            )
                            n_attack = 1 + sum(
                                [
                                    1
                                    for card in self.table.attack_cards
                                    if card == "Attack"
                                ]
                            )
                            for _ in range(2 * n_attack):
                                if (
                                    self.deck.deck
                                ):  # extracting a card from the deck and checking it
                                    card = self.deck.deck.pop(0)
                                    player.cards.append(card)
                                else:
                                    print(
                                        "No more cards"
                                    )  # WHAT TO DO IF THERE AREN'T ANY MORE CARDS?'
                            self.table.cards.extend(
                                self.table.attack_cards
                            )  # add the attack cards to the table cards
                            self.table.attack_cards = (
                                []
                            )  # empty the list of attack cards
                ...
                )  # I could add the fact that you could just use another 'Nope'
                answer = input()
                while answer not in ["y", "n"]:
                    print("You should enter a valid input")
                    print(
                ...
                if used_card == "Attack":
                    ...
                    while "Attack" in player.cards and answer == "y":
                        ...
                        if answer == "y":
                            ...
                            self.table.attack_cards.append(
                                player.cards.pop(index)
                            )
                    return print(f"{player.name}'s turn is over")

                # and so used_card in ['Favor', 'Cat Cards', 'See the Future', 'Shuffle', 'Skip']
                else:
                    print(f"You're using {used_card}")
                    if used_card == "Favor":
                        players_number_names = [
                            (el[0], el[1].name) for el in players_number
                        ]
                        print(
                    ...
                    if used_card == "Cat Cards":
                        cat_cards = [
                            ...
                            if player.cards[i] == "Cat Cards"
                        ]
                        if len(cat_cards) == 0:
                            print(
                                "To use this card you should have at least 2 cat cards in your hand!"
                            )
                            ...
                            print(
                                f"Which other card do you want to use? [Reply with a number]\n\nYour cards are {[(i+1, player.cards[i]) for i in range(len(player.cards))]}"
                            )
                            y = int(input())
                            ...
                            if len(player_choosen.cards) == 0:
                                ...
                                print(
                                    f"{player_choosen.name} has {len(player_choosen.cards)} cards. Which one do you want? [Reply with a number in 1-{len(player_choosen.cards)}]"
                                )
                                x = int(input())
                                choosen_card = player_choosen.cards.pop(x - 1)
                            ...
                            print(
                                f"Which card do you want to use? [Reply with two comma separated numbers]\n\nYour cards are {[(i+1, player.cards[i]) for i in range(len(player.cards))]}"
                            )
                            y = input().split(",")
                            for i in range(2):
                            ...
                            if len(player_choosen.cards) == 0:
                                ...
                                if choosen_card in player_choosen.cards:
                                    ...
                                    print(
                                        f"{player_choosen.name} doesn't have this card!"
                                    )

                    if used_card == "See the Future":
                        next_cards = [self.deck.deck[i] for i in range(3)]
                        print(
                            f"The next tree cards in the deck are {next_cards}"
                        )
        ...
        if self.deck.deck:  # extracting a card from the deck and checking it
            ...
            if card == "Exploding Kitten":
                ...
                if "Defuse" in player.cards:
                    ...
                    print(f"Now your cards are \n{player.cards}\n")
                    print(
                        f"You have to put the Exploding kitten back in the deck of cards. The deck has {len(self.deck.deck)} cards. \nIn which position do you want to put the bomb? [Reply with a number]"
                    )
                    test = True
                    while test:
                        try:
